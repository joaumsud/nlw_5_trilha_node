...
function call(id) {
    const connection = connectionsUsers.find((connections) => connections.socket.id === id);
    ...
    socket.emi("admin_user_in_support", params);
...
socket.on("admin_receive_message", data => {
    ...
    createDiv.className = "admin_message_client";
    createDiv.innerHTML = `<span>${connection.user.email}</span>`;
    createDiv.innerHTML += `<span> ${data.messagem.text}</span>`;
    createDiv.innerHTML += `<span class="admin_date">${dayjs(params.message.create_at).format("DD/MM/YYYY HH:mm:ss")}</span>`;

    divMessages.appendChicl(createDiv);
})